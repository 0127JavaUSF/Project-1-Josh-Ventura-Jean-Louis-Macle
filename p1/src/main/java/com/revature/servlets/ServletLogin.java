...
public class ServletLogin extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		ArrayList<Integer> userIdAndRoleId = dao.getUserIdAndRoleId(username, password);
		int userId = userIdAndRoleId.get(0);
		if (userId == 0) {
			//Display an error message
			response.getWriter().append("Wrong credentials. UserId:"+userId+" for username:"+username+" and password:"+password+".");
		}
		else {
			...
			session.setAttribute("roleId", userIdAndRoleId.get(1));
			...
			//userID, date submitted/resolved, description, amount, status
			  ...
			  om.writeValue(response.getWriter(), role);			  	
		}
	}
}	
