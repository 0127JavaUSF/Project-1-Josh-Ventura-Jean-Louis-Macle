...
public class ServletChangeStatus extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		String reimb_id = request.getParameter("reimbId"); 
		String status = request.getParameter("status"); 
		String resolver_id = request.getParameter("resolverId");
		ReimbursementDAO dao = new ReimbursementDAO();
		dao.changeReimbursementStatus(Integer.parseInt(reimb_id), Integer.parseInt(resolver_id), status);
		response.setStatus(200);
		//response.getWriter().append(reimb_id+","+status);
	}
